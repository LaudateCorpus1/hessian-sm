...
//    serializer.writeObject(object, this);
      if ( object instanceof External ) {
          int ref = writeObjectBegin(object.getClass().getName());
          if ( ref < -1 ) throw new IOException("not expect External interface for " + object.getClass().getName() );
          else {
              //write field lengh as 0 when it is new object definition
              if ( ref == -1 ) writeClassFieldLength(0);
              ((External) object).writeExternal( this );
          }
      }
      else {
